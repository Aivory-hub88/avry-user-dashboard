...
import type { AivoryErrorResponse } from '@/types/aivory'

interface WorkflowAivoryRefineModalProps {
  workflow: SavedWorkflow
  onClose: () => void
  onApply: (updatedWorkflow: SavedWorkflow) => void
}

export function WorkflowAivoryRefineModal({
  workflow,
  onClose,
  onApply,
}: WorkflowAivoryRefineModalProps) {
  ...
  const handleRefine = async () => {
    ...
    try {
      ...
      ;(window as any).__aivoryRefineWorkflow = response.updatedWorkflow
    } catch (err: any) {
      const errorData = err as any
      if (errorData?.errorCode) {
        const aivoryError = errorData as AivoryErrorResponse
      ...
      console.error('[WorkflowAivoryRefineModal] Error:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleApply = () => {
    const updatedWorkflow = (window as any).__aivoryRefineWorkflow
    if (updatedWorkflow) {
