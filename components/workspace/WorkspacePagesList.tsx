...
/**
 * WorkspacePagesList — daftar dokumen (Pages/Trash) yang sebelumnya hanya
 * hidup di /workspace. Diekstrak jadi komponen supaya bisa dipakai dua tempat:
 * - /workspace?view=pages (sub-page dokumen)
 * - /workspace/[id]?view=pages (tab Pages di pill bar doc view)
 *
 * Self-contained: fetch list + trash sendiri, search lewat prop `query`.
 */
export default function WorkspacePagesList({ query = "" }: { query?: string }) {
  const [docs, setDocs] = useState<PagesListDoc[]>([])
  const [trashDocs, setTrashDocs] = useState<PagesListDoc[]>([])
  ...
  const create = async () => {
    ...
    try {
      ...
      if (r.ok && j.id) {
        router.push(`/workspace/${j.id}?view=page`)
      }
      else if (r.status === 401) {
        setAuthRequired(true)
         ...
         {loading ? (
          ...
          <div className="py-8 text-center">
            ...
            <div className="mt-4 flex justify-center">
               <button onClick={() => create()} disabled={creating} className="rounded-full bg-white px-4 py-2 text-[12px] font-medium text-black disabled:opacity-50">Create your first page</button>
            </div>
          </div>
        ) : filtered.length === 0 ? (
          <div className="py-8 text-center text-[12px] text-white/30">No documents match “{query.trim()}”.</div>
          ) : (
