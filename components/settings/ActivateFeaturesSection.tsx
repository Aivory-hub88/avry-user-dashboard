      ...
      <div className="p-5 lg:p-6 flex flex-col flex-1 relative z-10">
        ...
        <div className="min-h-[75px] mb-2">
          <div className="flex justify-between items-start mb-3 h-5">
            {highlight && !isActive && (
              <span className="inline-flex items-center px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.15em] text-accent bg-accent/10 border border-accent/20 rounded-full">
                Most Popular
              </span>
            )}
            {isActive && (
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.15em] text-[#242424] bg-accent rounded-full">
          ...
          {subtitle && (
            <p className="text-[12px] font-medium text-accent mt-1.5">
              {subtitle}
            </p>
          )}
        </div>
        ...
        <div className="flex items-end justify-start gap-2.5 py-3 mb-1">
          ...
          <div className="flex flex-col pb-1">
            ...
            <div className={`w-full h-[3px] rounded-full ${isActive ? 'bg-accent' : highlight ? 'bg-accent/50' : 'bg-white/10'}`} />
          </div>
        </div>
        ...
        <ul className="space-y-2 mb-6 flex-1">
          {features.map((f: string, i: number) => (
            <li key={i} className="flex items-start gap-2.5 text-[13px] text-gray-300">
              <svg className={`w-4 h-4 shrink-0 mt-0.5 ${isActive ? 'text-accent' : 'text-gray-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
              </svg>
        ...
        {/* Savings */}
        {savings && (
          <div className="mb-5 inline-flex items-center justify-center bg-accent/10 border border-accent/20 px-3 py-1.5 rounded-lg text-[12px] font-semibold text-accent">
            {savings}
          </div>
        )}
          ...
          <button
            ...
            className={`w-full py-2.5 rounded-xl text-[13px] font-medium transition-all flex items-center justify-center gap-2 group/btn ${
              isActive
                ...
                : 'bg-gradient-to-b from-white/[0.09] to-white/[0.03] hover:from-accent/25 hover:to-accent/10 text-white/95 hover:text-white border border-white/10 hover:border-accent/30 shadow-[0_1px_0_rgba(255,255,255,0.08)_inset,0_4px_12px_rgba(0,0,0,0.25)]'
            }`}
          >
            {isLoading ? (
              <>
              ...
              <>
                {actionText}
                {!isActive && !isLoading && (
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5 opacity-0 -translate-x-1 group-hover/btn:opacity-100 group-hover/btn:translate-x-0 transition-all duration-200 text-accent">
