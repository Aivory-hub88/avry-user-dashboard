...
const compactMarkdownComponents = {
  ...
  a: ({ href, children }: any) => (
    <a href={href} className="text-accent hover:text-accent/80 underline underline-offset-2 transition-colors text-[13px]" target="_blank" rel="noopener noreferrer">
      {children}
    </a>
  ),
  ul: ({ children }: any) => (
  ...
  blockquote: ({ children }: any) => (
    <blockquote className="my-2 pl-3 border-l-2 border-accent/40 text-[#a1a1aa] italic text-[13px]">
      {children}
    </blockquote>
  ),
}
              ...
              <div>
                {isStreaming ? (
                  <p className="text-[13px] leading-[1.55] text-[#f7f7f7] whitespace-pre-wrap">
                    {content}
                    <span
                      className="inline-block w-[2px] h-[13px] bg-accent ml-0.5 align-middle rounded-sm"
