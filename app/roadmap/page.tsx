...
const T = {
  ...
  // real white — same hue family as `text` removes the colour cast).
  textSub:      'rgba(240,237,233,0.72)',
  textMuted:    'rgba(240,237,233,0.72)',
  red:          '#f87171',
...
// Framed around the business-operations transformation, not "AI" as the
// subject — AI/automation is the mechanism, not the headline.
...
// Reframed around business operations transformation as the subject — AI/
// automation is the tool the transformation uses, not what the roadmap is
// "about". Used to open with "prove AI works in your environment", which
// read as an AI-adoption plan rather than an operations transformation.
const PHASE_DESCRIPTIONS: Record<'en' | 'id', Record<number, string>> = {
  en: {
    0: 'Start here. The goal is to prove this transformation works in your environment by shipping 3 automated workflows in 90 days. Every 90 days of delay costs your organisation approximately $3,525 in unrealized savings, so speed matters more than perfection in this phase. Automated Reporting goes first because its 5-week time to value is the fastest of all modules, and the data pipelines it establishes become the foundation that CS Ticket Automation and Process Automation depend on. Phase 1 is complete when your team has three workflows running in production, at least 10 hours per week are being reclaimed, and your internal stakeholders can point to measurable proof that the transformation delivers results in your operation.',
    ...
    2: 'Shift from building to measuring. If your actual annual savings are tracking below the $14,296 target, check two factors first: the efficiency factor applied to each automated workflow and whether automation coverage has genuinely reached the projected 62.5%. The 80% team adoption rate target means more than login frequency — it means team members are incorporating the new tools into their daily workflow without being prompted, a behavioural shift that typically requires 4–6 weeks of reinforcement after deployment. Success at Month 12 builds the quantitative case for the next investment cycle, demonstrating that your $30,000 initial investment is on track to deliver a 43% return over three years.',
  },
  id: {
    0: 'Mulai di sini. Tujuannya adalah membuktikan bahwa transformasi ini berhasil di lingkungan Anda dengan meluncurkan 3 alur kerja terotomasi dalam 90 hari. Setiap keterlambatan 90 hari membebani organisasi Anda sekitar $3.525 dalam penghematan yang belum terealisasi, sehingga kecepatan lebih penting daripada kesempurnaan pada fase ini. Automated Reporting dijalankan lebih dulu karena waktu-ke-value 5 minggunya adalah yang tercepat dari semua modul, dan jalur data yang dibangunnya menjadi fondasi bagi CS Ticket Automation dan Process Automation. Fase 1 dianggap selesai ketika tim Anda memiliki tiga alur kerja yang berjalan di produksi, setidaknya 10 jam per minggu berhasil dibebaskan, dan pemangku kepentingan internal Anda dapat menunjukkan bukti terukur bahwa transformasi ini memberikan hasil nyata pada operasi Anda.',
    ...
    2: 'Beralih dari membangun ke mengukur. Jika penghematan tahunan aktual Anda berada di bawah target $14.296, periksa dua faktor terlebih dahulu: faktor efisiensi yang diterapkan pada setiap alur kerja terotomasi dan apakah cakupan otomasi benar-benar telah mencapai proyeksi 62,5%. Target tingkat adopsi tim 80% bermakna lebih dari sekadar frekuensi login — artinya anggota tim mengintegrasikan alat baru ke dalam alur kerja harian mereka tanpa perlu diminta, suatu perubahan perilaku yang biasanya membutuhkan 4–6 minggu penguatan setelah penerapan. Keberhasilan pada Bulan ke-12 membangun basis kuantitatif untuk siklus investasi berikutnya, menunjukkan bahwa investasi awal Anda sebesar $30.000 berada di jalur untuk memberikan imbal hasil 43% dalam tiga tahun.',
  },
};
