 ...
 * GET → newest first. POST { title?, body? } → new note (write role).
 */
import { NextRequest, NextResponse } from "next/server"
import { query } from "@/lib/db"
import { newId } from "@/lib/spaceWrite"
import { cleanNoteInput, noteFromRow, noteGate, NOTES_PER_ROOM } from "@/lib/roomNotes"
...
export async function POST(req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const r = await query(
      `INSERT INTO dashboard.room_notes (id, room_id, title, body, created_by, updated_by)
       VALUES ($1, $2, $3, $4, $5, $5) RETURNING *`,
      [newId(), id, v.title ?? "", v.body ?? "", g.who],
    )
