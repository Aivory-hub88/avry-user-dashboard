...
/**
 * POST /api/workflows/aivory-edit
 * AI-edit an existing workflow from a natural-language instruction.
 * Body:   { mode: 'EDIT_WORKFLOW', workflow, instruction }
  ...
  try {
    ...
    if (!r.ok) {
      return NextResponse.json(
        { errorMessage: `AI edit engine returned ${r.status}` },
        { status: 502 },
      )
    }
    ...
    return NextResponse.json(
      { errorMessage: err instanceof Error ? err.message : 'AI edit failed' },
      { status: 500 },
    )
  }
}
