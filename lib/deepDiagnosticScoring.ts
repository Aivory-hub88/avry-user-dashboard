...
const RUBRIC: Record<string, ScoredQuestion> = {
  ...
  data_centralization: { weight: 1.5, label: 'Data centralisation',
  ...
  workflow_standardization: { weight: 1.0, label: 'Workflow standardisation',
    scores: [1.0, 0.66, 0.33, 0.0] },
...
export function calculateReadinessScore(phases: PhasesPayload): ReadinessResult {
  ...
  for (const [qid, rule] of Object.entries(RUBRIC)) {
    ...
    if (idx < 0) continue // answer not recognised -> omit (don't guess)
  ...
  // No recognised answers -> neutral 50 (matches the free diagnostic's
